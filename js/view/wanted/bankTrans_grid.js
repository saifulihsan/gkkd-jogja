jun.bankTransStore = new Ext.data.JsonStore({
    //autoDestroy:true,
    root:'data',
    storeId:'bankTransStore',
    //scope:jun.NotaDtlGrid,
    url:'Wanted/bankTrans/view',
    //idProperty: 'item_id',
    fields:[
        {name:'type'},
        {name:'ref'},
...
//			width:100
    ...
    initComponent:function () {
        this.store = jun.bankTransStore;
        this.store.on({

            'load':{
                fn:function (store, records, options) {
                    //var tmp = store;
//                    var classBold = 'x-row-bold';
//                    Ext.fly(this.getView().getRow(1)).addClass(classBold);
//                    row.style.fontWeight = "Bold";
//                    var el = Ext.get(row);
//                    el.addClass(classBold);
//                    this.getView().refresh();
//                    this.getSelectionModel().selectRow(0);
                    //this.getStore().refreshData();
                    //this.getView().getRow(1).style.color="#f30";
...
jun.bankStatement = Ext.extend(Ext.Panel, {
    title:"Bank Statement",
    //html:'<p style="padding:25px">welcome to the jungle</p>',
...
//                    hiddenValue:'bank_act',
        ...
        jun.bankStatement.superclass.initComponent.call(this);
        this.on('activate', this.onActivate, this);
        this.on('resize', this.onAfterReder, this);
        this.btnRefresh.on('click', this.onbtnRefreshClick, this);
        Ext.getCmp('bank_act_banktrans').setValue(jun.rztBankAccounts.getAt('0').get('id'));
    },

    onAfterReder:function () {
        var h = Ext.getCmp('mainpanel').getHeight();
        var trans = Ext.getCmp('bankTransPanel');
        trans.setHeight(h - 55);
        trans.expand(false);
        trans.doLayout(false);
    },

    onActivate:function () {
        //aExt.getCmp('bankTransPanel').height = Ext.getCmp('mainpanel').getHeight();
//        Ext.getCmp('bankTransPanel').load({
//            url:'Wanted/BankTrans/view/',
//            scripts:true,
//            params:{
//                bank_act:Ext.getCmp('bank_act_banktrans').getValue(),
...
//        var grid = new Ext.ux.grid.TableGrid("table-bank-trans", {

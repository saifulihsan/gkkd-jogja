...
class PeBankTransController extends GxController
{
    ...
    public function actionGetBalance()
    {
        ...
        if (isset($_POST) && !empty($_POST)) {
            //$id = $_POST['id'];
            $amt = Pe::get_balance_before_for_bank_account(get_date_tomorrow(), $_POST['id']);
    ...
    public function actionCreateTransfer()
    {
        ...
        if (isset($_POST) && !empty($_POST)) {
            ...
            try {
                ...
                Pe::add_gl(BANKTRANSFER, $trans_no, $trans_date, $docref, $bank_account_tujuan,
                    $memo, $amount, $user);
                Pe::add_gl(BANKTRANSFER, $trans_no, $trans_date, $docref, $bank_account_asal, $memo, -$amount,
                    $user);
...
//    }
    ...
    {
        $model = $this->loadModel($id, 'PeBankTrans');
        if (isset($_POST) && !empty($_POST)) {
            foreach ($_POST as $k => $v) {
                $_POST['PeBankTrans'][$k] = $v;
            }
            $model->attributes = $_POST['PeBankTrans'];
            if ($model->save()) {
                $status = true;
            } else {
                $status = false;
    ...
    public function actionDelete($id)
    {
        if (Yii::app()->request->isPostRequest) {
            $this->loadModel($id, 'PeBankTrans')->delete();
            if (!Yii::app()->request->isAjaxRequest)
    ...
    public function actionAdmin() {
         $dataProvider = new CActiveDataProvider('PeBankTrans');

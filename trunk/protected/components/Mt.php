...
class Mt{
	...
	static function get_max_type_no($type){
		$type_no=app()->db->createCommand()->select("IFNULL(MAX(mt_gl_trans.type_no),0)")->from(
				"mt_gl_trans")->where('type=:type',array(
				':type'=>$type
		))->queryScalar();
		return $type_no;
	}
	...
	static function add_gl($type,$trans_id,$date_,$ref,$account,$memo_,$amount,$person_id,$id_mobil){
		$is_bank_to=Mt::is_bank_account($account);
		Mt::add_gl_trans($type,$trans_id,$date_,$account,$memo_,$amount,$person_id,$id_mobil);
		if($is_bank_to){
	...
	static function add_gl_trans($type,$trans_id,$date_,$account,$memo_,$amount,$person_id,$id_mobil){
	...
	static function get_sum_account_trans($account,$id_mobil,$from_date,$to_date){
		$comm_mobil='';
		if($id_mobil!=null){
	...
	static function update_comments($type,$id,$date_,$memo_){
		if($date_==null){
			...
			Mt::add_comments($type,$id,Yii::app()->dateFormatter->format('yyyy-MM-dd',time()),
					$memo_);
		}else{
			$criteria=new CDbCriteria();
			$criteria->addCondition("type=".$type);

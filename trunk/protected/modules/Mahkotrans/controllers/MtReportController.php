...
//                $kas_keluar = $row['kas_keluar'] == 0 ? '' : ($format == 'excel' ? -$row['kas_keluar'] : number_format(-$row['kas_keluar']));
                $objPHPExcel->setActiveSheetIndex(0)->setCellValue("A$start", $nomer)
                    ...
                    ->setCellValue("C$start", Mt::get_payee_payoor($row['type'], $row['trans_no']))
...
//                $objWriter->save('php://output');
    ...
    public function actionPengeluaranPerKodeRekening()
    {
        ...
        if (isset($_POST) && !empty($_POST)) {
            ...
            $start++;
            $rows = Mt::get_pengeluaran_per_kode_rekening($start_date, $end_date);
            $total = Mt::get_total_pengeluaran($start_date, $end_date);
            $total_persen = 0;
            foreach ($rows as $row) {
    ...
    public function actionPendapatan()
    {
        ...
        if (isset($_POST) && !empty($_POST)) {
            ...
            $objPHPExcel = new PHPExcel();
            $this->header($objPHPExcel, $start, $worksheet_name, "LAPORAN PENDAPATAN");
            $objPHPExcel->setActiveSheetIndex(0)->mergeCells("A$start:G$start")
                ->setCellValue("A$start", "PERIODE: " . sql2long_date($start_date) . " - " . sql2long_date($end_date))
                ->getStyle("A$start")->getFont()->setSize(12)->setBold(true);
            $objPHPExcel->getActiveSheet()->setTitle($worksheet_name);
            ...
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue("A$start", "Nama Rekening")
                ...
                ->getFont()->setBold(true);
            $start++;
            $sub_total = Mt::get_total_pendapatan($start_date, $end_date);
            $rows = Mt::get_detil_pendapatan($start_date, $end_date);
            $total_persen = 0;
            foreach ($rows as $row) {
    ...
    public function actionBebanAktivitas()
    {
        ...
        if (isset($_POST) && !empty($_POST)) {
            ...
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue("A$start", "Sub Aktivitas")
                ...
                ->getFont()->setBold(true);
            $start++;
            $rows = Mt::get_beban_aktivitas($start_date, $end_date);
            $total = Mt::get_total_beban_aktivitas($start_date, $end_date);
            $total_persen = 0;
            foreach ($rows as $row) {
                $total_persen += $row['total_beban'] / $total;
                $total_beban = $format == 'excel' ? $row['total_beban'] : acc_format($row['total_beban']);
    ...
    public function actionBebanAnak()
    {
        ...
        if (isset($_POST) && !empty($_POST)) {
            ...
            $start_date = $_POST['trans_date_mulai'];
            $end_date = $_POST['trans_date_sampai'];
            $anak_id = $_POST['mt_member_id'];
            $start = 1;
            $file_name = 'BebanAnak';
            $worksheet_name = 'BebanAnak';
            $objPHPExcel = new PHPExcel();
            $this->header($objPHPExcel, $start, $worksheet_name, "LAPORAN BEBAN AKTIVITAS PER ANAK");
            $objPHPExcel->setActiveSheetIndex(0)->mergeCells("A$start:G$start")
                ->setCellValue("A$start", "PERIODE: " . sql2long_date($start_date) . " - " . sql2long_date($end_date))
                ->getStyle("A$start")->getFont()->setSize(12)->setBold(true);
            $objPHPExcel->getActiveSheet()->setTitle($worksheet_name);
            ...
            $start++;
            $rows = Mt::get_beban_anak($start_date, $end_date, $anak_id);
            $total = Mt::get_total_beban_anak($start_date, $end_date, $anak_id);
            $total_persen = 0;
            foreach ($rows as $row) {
    ...
    public function actionBebanGrup()
    {
        ...
        if (isset($_POST) && !empty($_POST)) {
            ...
            $objPHPExcel = new PHPExcel();
            $this->header($objPHPExcel, $start, $worksheet_name, "LAPORAN BEBAN AKTIVITAS PER GRUP");
            $objPHPExcel->setActiveSheetIndex(0)->mergeCells("A$start:G$start")
                ->setCellValue("A$start", "PERIODE: " . sql2long_date($start_date) . " - " . sql2long_date($end_date))
                ->getStyle("A$start")->getFont()->setSize(12)->setBold(true);
            $objPHPExcel->getActiveSheet()->setTitle($worksheet_name);
            ...
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue("A$start", "Nama Grup")
                ...
                ->getStyle("A$start:C$start")->getFont()->setBold(true);
            $start++;
            $rows = Mt::get_beban_grup($start_date, $end_date, $anak_id);
            $total = Mt::get_total_beban_grup($start_date, $end_date, $anak_id);
            $total_persen = 0;
            foreach ($rows as $row) {
                $total_persen += $row['amount'] / $total;
                $total_beban = $format == 'excel' ? $row['amount'] : acc_format($row['amount']);
    ...
    public function actionTanggungJawab()
    {
        ...
        if (isset($_POST) && !empty($_POST)) {
            ...
            $objPHPExcel = new PHPExcel();
            $this->header($objPHPExcel, $start, $worksheet_name, "LAPORAN PERTANGGUNGJAWABAN");
            $objPHPExcel->setActiveSheetIndex(0)->mergeCells("A$start:G$start")
            ...
            $start++;
            $start_body = $start;
            $saldo_awal_kas = Mt::get_balance_before_for_bank_account("$tahun-$bulan-01");
            ...
            $start++;
            $total_pendapatan = Mt::get_total_pendapatan($start_date, $end_date);
            ...
            $start++;
            $saldo_akhir_kas = Mt::get_balance_before_for_bank_account("$tahun-" . ($bulan + 1) . "-01");
    ...
    public function actionAnggaranRealisasi()
    {
        ...
        if (isset($_POST) && !empty($_POST)) {
            ...
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue("A$start", "Nama Rekening")
                ...
                ->getFont()->setBold(true);
            $start++;
            $rows = Mt::get_chart_master_beban();
            $total_anggaran = 0;
            $total_realisasi = 0;
            foreach ($rows as $row) {
                $anggaran = Mt::get_anggaran_by_code($bulan, $tahun, $row['account_code']);
                $realisasi = Mt::get_realisasi_by_code($start_date, $end_date, $row['account_code']);
...
//            $lampiran_id = $_POST['id'];
            ...
            $objPHPExcel = new PHPExcel();
            $this->header($objPHPExcel, $start, $worksheet_name, "LAMPIRAN DONASI NON TUNAI");
            $objPHPExcel->setActiveSheetIndex(0)->mergeCells("A$start:G$start")
                ->setCellValue("A$start", "PERIODE: " . sql2long_date($start_date) . " - " . sql2long_date($end_date))
                ->getStyle("A$start")->getFont()->setSize(12)->setBold(true);
            $objPHPExcel->getActiveSheet()->setTitle($worksheet_name);
            ...
            $start++;
//            $rows = Mt::get_beban_anak($start_date, $end_date, $lampiran_id);
            $criteria = new CDbCriteria();
            $criteria->addBetweenCondition('trans_date', $start_date, $end_date);
            $rows = MtLampiran::model()->findAll($criteria);
            $no = 0;
            foreach ($rows as $row) {
                $no++;
    ...
    public function actionPengeluaranPerKodeRekeningDetil()
    {
        ...
        if (isset($_POST) && !empty($_POST)) {
            ...
            $daftar_code = Mt::get_arr_kode_rekening_pengeluaran($kode);
            $index = 0;
            $objPHPExcel = new PHPExcel();
            $objPHPExcel->getDefaultStyle()->getFont()->setSize(10);
            ...
            foreach ($daftar_code as $key => $codename) {
                ...
                $objPHPExcel->getActiveSheet()->setCellValue("A$start", "Tgl")->setCellValue("B$start", "Keterangan")
                    ...
                    ->getFont()->setBold(true);
                $start++;
                $rows = Mt::get_pengeluaran_detil_kode_rekening($start_date, $end_date, $key);
                $total = Mt::get_total_pengeluaran($start_date, $end_date, $key);
                $total_persen = 0;
                foreach ($rows as $row) {
//                    $total_persen += $row['total_beban'] / $total;

...
class MtReportController extends GxController
{
    ...
    protected function footer($objPHPExcel, $start, $file_name, $format, $html_title)
    {
        ...
        $start++;
        $objPHPExcel->setActiveSheetIndex(0)->mergeCells("A$start:G$start")->
            setCellValue("A$start", "Pada tanggal " . get_date_today('dd/MM/yyyy') . " jam " .
            get_time_now());
        ob_end_clean();
        ob_start();
        if ($format == 'excel') {
    ...
    public function actionMutasiKasPerBank()
    {
        ...
        if (isset($_POST) && !empty($_POST)) {
            ...
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue("A$start",
                "Type")->setCellValue("B$start", "Ref. Dokumen")->setCellValue("C$start",
                "Tanggal")->setCellValue("D$start", "Debit")->setCellValue("E$start",
                "Kredit")->setCellValue("F$start", "Saldo")->getStyle("A$start:F$start")
                ->getFont()->setBold(true);
            $start++;
            $arr = Mt::get_bank_trans_view();
            foreach ($arr['data'] as $row){
            	$objPHPExcel->setActiveSheetIndex(0)->setCellValue("A$start", $row['type'])->
            	setCellValue("B$start", $row['ref'])->setCellValue("C$start", sql2date($row['tgl']))->
            	setCellValue("D$start",$row['debit'])->setCellValue("E$start",$row['kredit'])->
    ...
    public function actionPenjualanPerMobil()
    {
        ...
        if (isset($_POST) && !empty($_POST)) {
            ...
            foreach ($rows as $row) {
                ...
                //                                2);
                $objPHPExcel->setActiveSheetIndex(0)->setCellValue("A$start", sql2date($row['trans_date']))->
                    setCellValue("B$start", $row['doc_ref'])->setCellValue("C$start", $format ==
            ...
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue("B$start", 'Total')->
                ...
                setCellValue("D$start", $format == 'excel' ? $total_disk : number_format($total_disk))->
                setCellValue("E$start", $format == 'excel' ? $total_netto : number_format($total_netto));
            $start++;
            $end_body = $start - 1;
            $styleArray = array('borders' => array('allborders' => array('style' =>
                            PHPExcel_Style_Border::BORDER_THIN)));

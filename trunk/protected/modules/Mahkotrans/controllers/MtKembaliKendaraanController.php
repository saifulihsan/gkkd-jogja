		...
		if(isset($_POST)&&!empty($_POST)){
			...
			try{
				...
				if($model->pelunasan>0){
					...
					Mt::add_gl(KEMBALI_KENDARAAN,$model->id_kembali,$date,$docref,$bank_account,'-',
							$model->pelunasan,$user,$model->idPinjam->id_mobil);
					Mt::add_gl(KEMBALI_KENDARAAN,$model->id_kembali,$date,$docref,
							Mt::get_prefs('akun_penjualan'),'-',-$model->pelunasan,$user,
				...
				try{
					...
					if($model->pelunasan>0){
						...
						Mt::add_gl(VOID,$void->id_voided,$date,$docref,
								Mt::get_prefs('akun_penjualan'),
								"VOID Pengembalian Kendaraan $docref",$model->pelunasan,$user,
								$model->idPinjam->id_mobil);

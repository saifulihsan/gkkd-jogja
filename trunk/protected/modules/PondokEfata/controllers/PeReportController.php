...
class PeReportController extends GxController
{
    public function actionMutasiKasDiTangan()
    {
        ...
        if (isset($_POST) && !empty($_POST)) {
            ...
            $objPHPExcel->setActiveSheetIndex(0)
                ->mergeCells("A$start:G$start")
                ->setCellValue("A$start", "PONDOK EFATA")
...
//                $objWriter->save('php://output');
    ...
    public function actionTanggungJawab()
    {
        ...
        if (isset($_POST) && !empty($_POST)) {
            ...
            $start++;
            $start_body = $start;
            $saldo_awal_kas = Pe::get_balance_before_for_bank_account("$tahun-$bulan-02");
...
//            $lampiran_id = $_POST['id'];
            ...
            $start++;
//            $rows = Pe::get_beban_anak($start_date, $end_date, $lampiran_id);
